           ...
           MPI_Comm comm /* in */ )
{
  ...
  MPI_Allgather (local_x, blocksize, MPI_DOUBLE,
         x, blocksize, MPI_DOUBLE, comm);
  ...
  for (k = 0; k < t; k++) {
    ...
    for (local_i = 0; local_i < blocksize; local_i++) {
      ...
      //if(matrix_type == UPPER_TRIANGULAR) start++; 
    }
    for (local_i = 0; local_i < blocksize; local_i++) {
  ...
  // MPI_Allgather (global_x, blocksize, MPI_DOUBLE,
  //            global_x, blocksize, MPI_DOUBLE, comm);
