...
void print_itmv_sample ( char *msgheader, double A[],  double x[], double d[], double  y[],
			 int  matrix_type, int n, int t) {
...
void print_itmv_sample_distributed ( char *msgheader, double local_A[],  double local_x[], double local_d[], double  local_y[],
				     int matrix_type, int n, int t, int blocksize) {
  int i, local_i;
  ...
  for (i = 0; i < n; i++){
    if(procmap(i,blocksize)==my_rank){
      ...
      printf("%s Proc %d x[%d] locally x[%d] = %f; d[%d] locally d[%d] = %f\n",
	     msgheader, my_rank, i, local_i, local_x[local_i], i, local_i, local_d[local_i]);
      printf("%s Proc %d Row A[%d] locally A[%d] last 4 elements = %f, %f, %f, %f\n",
	     msgheader, my_rank, i, local_i, local_A[local_i*n+n-4], 
	     local_A[local_i*n+n-3], local_A[local_i*n+n-2], local_A[local_i*n+n-1]); 
    }
  }
...
char*  validate_vect(char *msgheader, double global_x[], int n, int t, int matrix_type){
  int i;
  double *expected;
  if(n <=0 )
  ...
  for (i = 0; i < n; i++){
#ifdef DEBUG1
    printf("%s Proc 0: i=%d  Expected %f Actual %f\n", msgheader, i, expected[i], global_x[i]);
#endif
    mu_assert("One mismatch in iterative mat-vect multiplication", global_x[i] ==expected[i]);
  }
  free(expected);
  return NULL;
}
...
int allocate_space( double **local_A, double **local_x, double **local_d, double **local_y, double **global_x, int blocksize, int n){
  ...
  *global_x=malloc(n*sizeof(double));
  /*Here we assume none of them are NULL. A more robust program needs to check the failed memory allocation*/
  if (*local_A == NULL || *local_x == NULL || *local_d==NULL ||  *local_y == NULL || *global_x==NULL) {
 ...
 */
int init_matrix( double *local_A, double *local_x, double *local_d, double *local_y, 
		 int blocksize, int n, int matrix_type, int my_rank){

  if (local_A == NULL || local_x == NULL || local_d==NULL ||  local_y == NULL ||blocksize<=0) 
    return FAIL;
 ...
 * If successful, return NULL
 */
char * itmv_test(char *testmsg, int test_correctness, int n, int matrix_type, int  t){
  ...
  succ= init_matrix( local_A, local_x, local_d, local_y, blocksize, n, matrix_type, my_rank);

#ifdef DEBUG1
  print_itmv_sample_distributed(testmsg, local_A, local_x, local_d, local_y, matrix_type, n, t, blocksize);
#endif
  ...
  free(local_d);
  free(global_x);
  return msg; /*Only process 0 conducts correctness test, and prints summary report*/
}
...
char * itmv_test9() {
  return itmv_test("Test 9: n=2K t=1K", !TEST_CORRECTNESS, 2048, !UPPER_TRIANGULAR, 1024);
}
char * itmv_test10() {
  return itmv_test("Test 10: n=2K t=1K upper", !TEST_CORRECTNESS, 2048, UPPER_TRIANGULAR, 1024);
}
char * itmv_test11() {
  return itmv_test("Test 11: n=4K t=1K", !TEST_CORRECTNESS, 4096, !UPPER_TRIANGULAR, 1024);
}
char * itmv_test12() {
  return itmv_test("Test 12: n=4K t=1K upper", !TEST_CORRECTNESS, 4096, UPPER_TRIANGULAR, 1024);
}
char * itmv_test13() {
  return itmv_test("Test 13: n=8K t=1K", !TEST_CORRECTNESS, 4096*2, !UPPER_TRIANGULAR, 1024);
}
char * itmv_test14() {
  return itmv_test("Test 14: n=8K t=1K upper", !TEST_CORRECTNESS, 4096*2, UPPER_TRIANGULAR, 1024);
}
/*-------------------------------------------------------------------
