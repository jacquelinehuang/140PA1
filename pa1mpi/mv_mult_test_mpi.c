...
char*  validate_vect(double y[], int blocksize, int n, int c){
		...
		if(procmap(i, blocksize)==my_rank){
			...
			printf("Proc %d: i=%d local_i=%d Expected %f Actual %f\n", my_rank, i, local_i,
				  expected, y[local_i]);
#endif
                	mu_assert("One mismatch in mat. vect. multiplication", y[local_i] ==expected);
		}
        }
        return NULL;
}
 ...
 * In args:
 *   	*local_A is the starting address of local space of matrix A with rowwise block mapping
 *   	*local_x is the starting address of local space of vector x with rowwise block mapping
 *   	*local_y is the starting address of local space of vector y with rowwise block mapping
 * 	blocksize is the size of local array 
 * 	n is the number of columns (and rows)
 */
 
int allocate_space( double **local_A, double **local_x, double **local_y, int blocksize, int n){
...
char * mv_test(char *testmsg, int n, int  c){
	...
	/*Initialize test matrix and vectors*/
   	...
   	if (my_rank==0)
        ...
        succ = mat_vect_mult(local_A, local_x, local_y,blocksize, n, my_rank, no_proc, comm);
...
char * mv_test1() {
   return mv_test("Test 1", 100, 1);
}
char * mv_test2() {
   return mv_test("Test 2", 1024, 2);
}
/*-------------------------------------------------------------------
